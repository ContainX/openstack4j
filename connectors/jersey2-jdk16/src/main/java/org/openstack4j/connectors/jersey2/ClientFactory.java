     ...
     */
     static Client create(Config config) {
         try {
             ...
             throw new ConnectionException("Issue creating Jersey 2 JDK16 Client: " + e.getMessage(), 0, e);
         }
     }
     ...
     private static Client buildClientFromConfig(Config config) {
         ...
         if (config.getProxy() != null) {
             cb.property(ClientProperties.PROXY_URI, config.getProxy().getHostWithPort());
         }
