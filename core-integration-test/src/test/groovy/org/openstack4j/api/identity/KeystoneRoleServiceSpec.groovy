...
class KeystoneRoleServiceSpec extends AbstractSpec {
    ...
    def "role service test cases: grant, revoke, check, list userroles in domain and project context"() {
        ...
        then: "the role list should contain one item"
        ...
        ActionResponse response_grantProjectUserRole_success = os.identity().roles().grantProjectUserRole(PROJECT_ID, ROLE_CRUD_USER_ID, ROLE_CRUD_ROLE_ID)
        ...
        ActionResponse response_checkProjectUserRole_success = os.identity().roles().checkProjectUserRole(PROJECT_ID, ROLE_CRUD_USER_ID, ROLE_CRUD_ROLE_ID)
        ...
        ActionResponse response_revokeProjectUserRole_success = os.identity().roles().revokeProjectUserRole(PROJECT_ID, ROLE_CRUD_USER_ID, ROLE_CRUD_ROLE_ID)
        ...
        //        ActionResponse response_userProjectRole_fail = os.identity().roles().checkProjectUserRole(PROJECT_ID, ROLE_CRUD_USER_ID, ROLE_CRUD_ROLE_ID)
        //
        ...
        ActionResponse response_grantDomainUserRole_success = os.identity().roles().grantDomainUserRole(DOMAIN_ID, ROLE_CRUD_USER_ID, ROLE_CRUD_ROLE_ID)
        ...
        ActionResponse response_checkDomainUserRole_success = os.identity().roles().checkDomainUserRole(DOMAIN_ID, ROLE_CRUD_USER_ID, ROLE_CRUD_ROLE_ID)
        ...
        when: "we revoke that role from the user"
        ActionResponse response_revokeDomainUserRole_success = os.identity().roles().revokeDomainUserRole(DOMAIN_ID, ROLE_CRUD_USER_ID, ROLE_CRUD_ROLE_ID)
        ...
        //  TODO: this test is disabled due to a malformed response returned by OpenStack as described in issue #530
        //        when: "we check again if the user is assigned to that role"
        //        ActionResponse response_checkDomainUserRole_fail = os.identity().roles().checkProjectUserRole(DOMAIN_ID, ROLE_CRUD_USER_ID, ROLE_CRUD_ROLE_ID)
        //
        //        then: "the response should indicate that we revoked the role before"
