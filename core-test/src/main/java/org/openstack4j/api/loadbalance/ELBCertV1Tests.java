...
public class ELBCertV1Tests extends AbstractTest {
	...
	public void testCreateCert() throws IOException {
		...
		Certificate cert = ELBCertificate.builder().name("SDK-test-cert").description("desc")
				...
				.build();
		Certificate create = osv3().elasticLoadBalance().certs().create(cert);
	...
	public void testDeleteCert() {
		...
		ActionResponse resp = osv3().elasticLoadBalance().certs().delete(certificateId);
	...
	public void testUpdateCert() throws IOException {
		...
		Certificates certs = osv3().elasticLoadBalance().certs().list();
		...
		Certificate afterUpdate = osv3().elasticLoadBalance().certs().update(cert.getId(), update);
