...
public class MemberTests extends AbstractTest{
	public void testListMember(){
		List<? extends Member> list = os().networking().member().list();
	...
	public void testCreateMember(){
		...
		Member create = Builders.member()
				...
				.weight(weight)
				.poolId(poolId).build();
		Member result = os().networking().member().create(create);
	...
	public void testUpdateMember(){
		...
		MemberUpdate update = Builders.memberUpdate()
				...
				.weight(weight)
				.poolId(poolId).build();
		Member result = os().networking().member().update(memberId, update);
		assertTrue(result.isAdminStateUp());
		assertEquals(weight, result.getWeight());
		assertEquals(address,result.getAddress());
	}
	...
	public void testDeleteMember(){
		String id = "99a28283-97b7-4210-af5c-b2594628217b";
		ActionResponse result = os().networking().member().delete(id);
