...
public class SchedulerStatsGetPoolTests extends AbstractTest {
    ...
    public void pools() throws Exception {
        ...
        List<? extends VolumeBackendPool> pools = os().blockStorage().schedulerStatsPools().pools();
    ...
    public void poolsDetail() throws Exception {
        ...
        List<? extends VolumeBackendPool> pools = os().blockStorage().schedulerStatsPools().poolsDetail();
        ...
        assertEquals((int) pool4.getCapabilities().getTotalCapacityGb(), 1024);
        assertEquals((int) pool4.getCapabilities().getReservedPercentage(), 0);
        assertEquals((int) pool4.getCapabilities().getFreeCapacityGb(), 100);
        ...
        assertEquals((int) pool5.getCapabilities().getTotalCapacityGb(), 512);
        assertEquals((int) pool5.getCapabilities().getReservedPercentage(), 0);
        assertEquals((int) pool5.getCapabilities().getFreeCapacityGb(), 200);
